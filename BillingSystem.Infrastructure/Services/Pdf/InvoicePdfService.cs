...
namespace BillingSystem.Infrastructure.Services.Pdf
{
    public class InvoicePdfService : IPdfService
    {
        ...
        public InvoicePdfService(IInvoiceRepository invoiceRepository, IConfiguration configuration)
        {
        ...
        public async Task<byte[]> GenerateInvoicePdfAsync(int invoiceId)
        {
            ...
            var invoice = await _invoiceRepository.GetByIdWithDetailsAsync(invoiceId, 0); // Note: userId=0 لأن PDF service لا يعتمد على user context
            ...
            // Load Logo
            byte[]? logoBytes = null;
            var webRootPath = _configuration["WebRootPath"] ?? Path.Combine(Directory.GetCurrentDirectory(), "wwwroot");
