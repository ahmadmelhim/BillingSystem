...
public class InvoiceService : IInvoiceService
{
        ...
        DateTime? toDate = null)
    {
        ...
        _logger.LogInformation("GetPagedAsync called - UserId: {UserId}, Search: {Search}, Status: {Status}", 
            currentUserId, search ?? "null", status ?? "null");
        
        if (!currentUserId.HasValue)
        {
            _logger.LogWarning("No current user found - returning empty list");
            return (Array.Empty<Invoice>(), 0);
        }

        var result = await _invoiceRepository.GetPagedAsync(currentUserId.Value, pageIndex, pageSize, search, status);
        
        // 🔍 DEBUG LOGGING - Remove after testing
        _logger.LogInformation("GetPagedAsync result - Items count: {Count}, Total: {Total}", 
